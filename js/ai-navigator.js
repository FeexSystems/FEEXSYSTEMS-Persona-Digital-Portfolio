...
export class AINavigator {
  constructor({ graph=buildKnowledgeGraph(), model=buildWorldModel(PERSONA_REGISTRY) }={}) { this.graph=graph; this.model=model; this.worlds=PERSONA_REGISTRY.worlds; saveWorldModel(model); }
  ...
  find(query='') { const q=query.trim().toLowerCase(); if(!q)return []; const terms=q.split(/\s+/).filter(Boolean); return this.nodes().filter(n=>`${n.label||''} ${n.domain||''} ${n.description||''} ${n.type||''}`.toLowerCase().includes(q)||terms.some(t=>`${n.label||''} ${n.domain||''} ${n.description||''}`.toLowerCase().includes(t))); }
  answer(input='') {
...
export function mountNavigator({input,output,onWorld,onRelations=()=>{},onPath=()=>{},navigator=null}){const nav=navigator||new AINavigator();const run=()=>{const result=nav.answer(input.value);output.innerHTML=`<b>${result.title}</b><p>${result.text}</p>`+(result.worlds.length?`<div class="nav-worlds">${result.worlds.map(w=>`<button data-world="${w.id}">${w.name}<small>${w.domain}</small></button>`).join('')}</div>`:'')+(result.nodes?.length?`<div class="nav-entities">${result.nodes.slice(0,8).map(n=>`<span>${n.type} · ${n.label}</span>`).join('')}</div>`:'');output.hidden=false;onRelations(result.relations||[]);onPath(result.path||[]);output.querySelectorAll('[data-world]').forEach(b=>b.onclick=()=>onWorld(b.dataset.world));};input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();run()}});return {run,nav};}
